 ...
 $sl_sanpham = "select * from sanpham  WHERE AnHien=1 and idSP <>1 ORDER BY SoLanMua DESC";
 ...
 <?php
 $result = mysqli_query($conn, 'select count(idSP) as total from sanpham  WHERE AnHien=1 and idSP <>1 ORDER BY SoLanMua DESC');
 ...
 $result = mysqli_query($conn, "SELECT * FROM sanpham   WHERE AnHien=1 and idSP <>1 ORDER BY SoLanMua DESC LIMIT $start, $limit");
 ?>
 <div class="row col-md-offset-4"><h3>Sản phẩm bán chạy </h3></div>
     ...
     <div class="row" align="center">
         <div class="pagination" >
             ...
             if ($current_page > 1 && $total_page > 1){
                 echo '<li><a href="index.php?index=3&page='.($current_page-1).'">Prev</a> </li> ';
             }
             for ($i = 1; $i <= $total_page; $i++){
                 if ($i == $current_page){
                     ...
                     echo '<li><a href="index.php?index=3&page='.$i.'">'.$i.'</a> </li> ';
                 }
             }
             if ($current_page < $total_page && $total_page > 1){
                 echo '<li><a href="index.php?index=3&page='.($current_page+1).'">Next</a> </li> ';
             }
             echo "</ul>";
             ?>
         </div>
